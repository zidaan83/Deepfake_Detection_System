...
if torch.cuda.is_available():
    device = 'cuda'          # correct string for NVIDIA GPU
...
class validation_dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for i, frame in enumerate(self.frame_extract(video_path)):
            # No face cropping – use full frame
...
def predict_page(request):
    if request.method == "GET":
        ...
        model = Model(2).to(device)  # .to(device) is cleaner and works for both

        model_name = get_accurate_model(sequence_length)
        path_to_model = os.path.join(settings.PROJECT_DIR, 'models', model_name)
        model.load_state_dict(torch.load(path_to_model, map_location=device))  # use device here too
        model.eval()
        ...
        # Process each frame (full frame only)
        for i in range(sequence_length):
            ...
            # Save full frame as preprocessed image
